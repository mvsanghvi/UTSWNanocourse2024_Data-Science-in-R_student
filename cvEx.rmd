...
card <- read_delim(knowingYourData_Filepath, delim = ";")
```
...
#Look at the frist 7 values of the dataset
card %>% head(7)
...
card%>%
  ggpairs(aes(color= as_factor(cholesterol)), progress = FALSE)
#setting color to numerical value will throw up an error, because lots of different weights and gets overwhelmed assigning new color to each value (i.e. weight)
```
...
#2 Drop duplicate features if any are present in your dataset
unique.card <- card[!duplicated(card),]
...
```{r}
#even though bands every value of 5, still continue to treat it as continuous
```
...
People have higher low pressure values than their high pressure values
...
Overfitting. Set aside data to exclusively test data after training
...
Different variables have different scales so are easier to compare

3) Do we scale our continous variables before or after splitting into training and test sets? Why? 
After. If done before, test set has knowledge of previous information and mean (feature leakage)
